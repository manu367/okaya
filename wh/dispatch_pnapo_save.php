...
if ($_POST['dispatchpo'] == "Dispatch" && $docid != '' && $_SESSION['asc_code'] != '') {
    ...
    if ($messageIdent_whdispatch != $sessionMessageIdent_whdispatch) {
        $_SESSION['messageIdent_whdispatch'] = $messageIdent_whdispatch;
        $flag = true;
        mysqli_autocommit($link1, false);
        $error_msg = "";
        $doctype_flag = 1;
        $sql_po = "SELECT from_code,to_code,potype FROM po_master where po_no='" . $docid . "'";
        $pomaster = mysqli_fetch_assoc(mysqli_query($link1, ));
        ...
        if (is_uploaded_file($_FILES['handset_img']['tmp_name'])) {
            ...
            if (!in_array($ext, $allowed)) {
                ...
                $my = "../uploads/billing/images/" . date("Y-M");
                if (!is_dir($my)) {
                    $s = mkdir($my, 0777, 'R');
                }
                ...
                if ($img_upload) {
                    $attach_a = $targetPath;
                    mysqli_query($link1, "INSERT INTO upload_history SET doc_no='" . $invoice_no . "', process='DISPATCH - PO/PNA', file_url='" . $targetPath . "', create_by='" . $_SESSION['userid'] . "', create_dt='" . $datetime . "', create_ip='" . $ip . "'");
                }
            }
        }
        ############################################
        ...
        if (is_uploaded_file($_FILES['att_video']['tmp_name'])) {
            ...
            if (!in_array($ext, $allowed)) {
                ...
                $my = "../uploads/billing/videos/" . date("Y-M");
                if (!is_dir($my)) {
                    $s = mkdir($my, 0777, 'R');
                }
        ...
        if ($doctype_flag == 1) {
            ...
            while ($podata_row = mysqli_fetch_assoc($podata_res)) {
                ...
                $post_part_nm = "part" . $podata_row['id'];
                ///// if post dispatch qty is more than zero
                if ($_POST[$post_dispqty] > 0) {
                    /////check inventory again
                    $avlqty = getInventory($_SESSION['asc_code'], $_POST[$post_part_nm], "okqty", $link1);
                    if ($avlqty >= $_POST[$post_dispqty]) {
                        ////////check hsn code of part
                        $res_part = mysqli_query($link1, "SELECT hsn_code,part_name FROM partcode_master where partcode='" . $_POST[$post_part_nm] . "' and status='1'");
                        ...
                        $sql_billdata = "INSERT INTO billing_product_items set from_location='" . $pomaster['to_code'] . "', to_location='" . $pomaster['from_code'] . "',challan_no='" . $invoice_no . "',request_no='" . $docid . "',job_no='" . $podata_row['job_no'] . "',type='" . $pomaster['potype'] . "',hsn_code='" . $row_part['hsn_code'] . "',partcode='" . $_POST[$post_part_nm] . "',part_name='" . $row_part['part_name'] . "',qty='" . $_POST[$post_dispqty] . "',okqty='" . $_POST[$post_dispqty] . "',price='" . $_POST[$post_price] . "',uom='PCS',value='" . $linetotal . "',basic_amt='" . $linetotal . "',cgst_per='" . $cgst_per . "',cgst_amt='" . $cgst_val . "',sgst_per='" . $sgst_per . "',sgst_amt='" . $sgst_val . "',igst_per='" . $igst_per . "',igst_amt='" . $igst_val . "',item_total='" . $tot_val . "',stock_type='okqty'";
                        ...
                        //----------------------------- po data upadate------------------------//
                        if (($_POST[$post_dispqty] + $podata_row['processed_qty']) == $podata_row['qty']) {
                            $linestatus = 2;
                        } else {
                            $linestatus = 6;
                        }
                        ...
                        //----------------------------- inventory upadate------------------------//
                        $sql_invt = "UPDATE client_inventory set okqty = okqty - '" . $_POST[$post_dispqty] . "',updatedate='" . $datetime . "' where partcode='" . $_POST[$post_part_nm] . "' and location_code='" . $pomaster['to_code'] . "' and  okqty >= '" . $_POST[$post_dispqty] . "'";
                        ...
                        if (mysqli_num_rows(mysqli_query($link1, "select partcode from client_inventory where partcode='" . $_POST[$post_part_nm] . "' and location_code='" . $pomaster['from_code'] . "'")) > 0) {
                            ///if product is exist in inventory then update its qty
                            $result = mysqli_query($link1, "update client_inventory set  	in_transit= in_transit+'" . $_POST[$post_dispqty] . "',updatedate='" . $datetime . "' where partcode='" . $_POST[$post_part_nm] . "' and location_code='" . $pomaster['from_code'] . "'");
                        } else {
                            //// if product is not exist then add in inventory
                            $result = mysqli_query($link1, "insert into client_inventory set location_code='" . $pomaster['from_code'] . "',partcode='" . $_POST[$post_part_nm] . "',in_transit='" . $_POST[$post_dispqty] . "',updatedate='" . $datetime . "'");
                        }
                        ...
                        if ($_POST[$post_dispqty] > 0) {
                            $flag = stockLedgerO($invoice_no, $today, $_POST[$post_part_nm], $_SESSION['asc_code'], $pomaster['from_code'], "OUT", "OK", "PO Dispatch", "Process", $_POST[$post_dispqty], $_POST[$post_price], $_SESSION['userid'], $today, $currtime, $_SERVER['REMOTE_ADDR'], $link1, $flag, $_SESSION['asc_code']);
                        }
            ...
            $sql_billmaster = "INSERT INTO billing_master set from_location='" . $pomaster['to_code'] . "', to_location='" . $pomaster['from_code'] . "', from_gst_no='" . $fromlocdet[8] . "', to_gst_no='" . $tolocdet[8] . "',
party_name='" . $tolocdet[0] . "', challan_no='" . $invoice_no . "', po_no='" . $docid . "', sale_date='" . $today . "', entry_date='" . $today . "',entry_time='" . $currtime . "', docket_no='" . $_POST['docket_no'] . "', courier='" . $_POST['courier_name'] . "', logged_by='" . $_SESSION['userid'] . "', billing_rmk='Against " . $pomaster['potype'] . "', bill_from='" . $fromlocdet[0] . "', from_stateid='" . $fromlocdet['5'] . "', to_stateid='" . $tolocdet[5] . "' ,bill_to='" . $tolocdet[0] . "', from_addrs='" . $fromlocdet[1] . "', disp_addrs='" . $fromlocdet[2] . "', to_addrs='" . $tolocdet[1] . "', deliv_addrs='" . $tolocdet[3] . "',status='2', document_type='" . $_POST['doc_type'] . "', finvoice_no='" . $_POST['finvoice_no'] . "', po_type='" . $pomaster['potype'] . "', basic_cost='" . $basic_cost . "',  total_cost='" . $inv_tot_cost . "', cgst_amt='" . $cgst_final_val . "', sgst_amt='" . $sgst_final_val . "', igst_amt='" . $igst_final_val . "', attach_a='" . $attach_a . "', attach_b='" . $attach_b . "'";
            ...
            //if(($total_reqqty == $total_qty) && $total_reqqty !=0 && $total_qty !=0){ $status = 2;}else{ $status = 6;}
            if ($_POST['tot_pend_qty'] == $total_qty) {
                $status = 2;
            } else {
                $status = 6;
            }
        ...
        mysqli_close($link1);
        ///// move to parent page
        header("location:invoice_list.php?msg=" . $msg . "&chkflag=" . $cflag . "&chkmsg=" . $cmsg . "" . $pagenav . "&challanno=" . $invoice_no . "&tolocation=" . $pomaster[from_code]);
        exit;
    }
    else {
