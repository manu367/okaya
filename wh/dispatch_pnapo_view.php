...
<head>
 ...
 <script>
	...
	function calculatetotal(){
		...
		function getAblQty(indx){
		var stock_type = document.getElementById("stock_type"+indx).value;
		var frm_code = document.getElementById("frm_code"+indx).value;
		var part = document.getElementById("part"+indx).value;
		var ses_code =  document.getElementById("sess_code").value;
		$.ajax({
			type:'post',
			url:'../includes/getAzaxFields.php',
			data:{stock_type_val:stock_type,frm_code:frm_code,part:part,ses_code:ses_code,index:indx},
			success:function(data){
				var getValue = data.split("~");
				document.getElementById("asp_va"+getValue[2]).value=getValue[0];
				document.getElementById("avl_qty"+getValue[2]).value=getValue[1];
			}
	    });
	}
	...
	////// get part details //////
	function getChangedPartInfo(partcode,indx){
		//alert('aaaaaa');
		$.ajax({
			type:'post',
			url:'../includes/getAzaxFields.php',
			data:{disPnaPoF:partcode,indx:indx,sess_code:'<?=$_SESSION['asc_code']?>',loc_code:'<?=$job_row['from_code']?>'},
			success:function(data){
				var getValue = data.split("~");
				//alert(getValue);
				...
				<tr>
					<td>
						<label class="custom_label">
							Image Attachement<br><span style="color:#ff5f5f;font-weight:normal;">( png, jpg, pdf | max: 3 MB )</span>
						</label>
					</td>
					<td>
					...
					<td>
						<label class="custom_label">
							Video Attachement <br><span style="color:#ff5f5f;font-weight:normal;">( mp4 | max: 10MB )</span>
						</label>
					</td>
					<td>
       ...
       <table class="table table-bordered" width="100%">
            <thead>
              <tr>
                ...
                <th width="15%">Requested Part</th>
                <th width="15%">Dispatch Partcode</th>
                <th width="10%">Price</th>
                <th width="8%">Req./Pend. Qty</th>
				 <th width="8%">ASP Avl Qty</th>
			...
			while($podata_row=mysqli_fetch_assoc($podata_res)){
				...
				$part=explode("~",getAnyDetails($podata_row['partcode'],"part_name,location_price,l2_price,l3_price,l4_price,l5_price","partcode","partcode_master",$link1));
				///// get available inventory
				$whavl = getInventory($_SESSION['asc_code'],$podata_row['partcode'],"okqty",$link1);
				...
				$aspavl = getInventory($job_row['from_code'],$podata_row['partcode'],"okqty",$link1);
				if($aspavl==""){
						$aspavlqty=0;
				}else{
			...
			?>
              <tr>
                ...
                <td><?=$brand[0]?></td>
                <td><?=$model[0]?></td>
                <td><?=$podata_row['job_no']?></td>
                <td><?php
				if($podata_row['job_no']!=''){
					$jobdata="SELECT warranty_status FROM jobsheet_data where job_no='".$podata_row['job_no']."'";
			$jobdata_res=mysqli_query($link1,$jobdata);
			$jobdata_row=mysqli_fetch_assoc($jobdata_res);
			
               echo $jobdata_row['warranty_status'];
			   } ?></td>
                <td><?php  echo $part[0]." (".$podata_row['partcode'].")"?></td>
                <td><select style="width: 200px;" name="chg_part<?=$podata_row['id']?>" id="chg_part<?=$i?>"  class="form-control"  onChange="getChangedPartInfo(this.value,<?=$i?>)" >
                    <option value=''>--Select Partcode--</option>
                    <?php
                        echo $dept_query="SELECT partcode,part_name,vendor_partcode FROM partcode_master where status = '1' and brand_id = '".$podata_row['brand_id']."' and product_id='".$podata_row['product_id']."' and model_id Like '%".$podata_row['model_id']."%'  order by partcode";
                        $check_dept=mysqli_query($link1,$dept_query);
                        while($br_dept = mysqli_fetch_array($check_dept)){
                      ?>
                    <option value="<?=$br_dept['partcode']?>"<?php if($podata_row['partcode'] == $br_dept['partcode']){ echo "selected";}?>><?php echo $br_dept['part_name']." | ".$br_dept['vendor_partcode']." | ".$br_dept['partcode']?></option>
                    <?php }?>
                  </select></td>
                <td align="right"><?=currencyFormat($price_asp)?><input name="price<?=$podata_row['id']?>" id="price<?=$i?>" type="hidden" value="<?=$price_asp?>"/>
                  <input type="hidden" name="part<?=$podata_row['id']?>" id="part<?=$i?>" value="<?=$podata_row['partcode'];?>" />
                  <input type="hidden" name="ship_address<?=$podata_row['id']?>" id="ship_address<?=$i?>" value="<?=$podata_row['ship_address'];?>" />
                  <input type="hidden" name="frm_code<?=$podata_row['id']?>" id="frm_code<?=$i?>" value="<?=$job_row['from_code'];?>" />
                  <input type="hidden" name="sess_code" id="sess_code" value="<?=$_SESSION['asc_code']?>" /></td>
                <td align="right"><?=$podata_row['qty']." / ".$pend_qty?><input name="req_qty<?=$podata_row['id']?>" id="req_qty<?=$i?>" type="hidden" value="<?=$pend_qty?>"/></td>
				 <td><input style="width: 50px;" type="text" name="asp_va<?=$podata_row['id']?>" id="asp_va<?=$i?>" value="<?=$aspavlqty;?>" class="form-control" readonly /></td>
            ...
            <tr>
			 ...
			 <tr>
                <td align="center" colspan="14"><span id="errormsg" class="red_small"></span>
...
<input name="tot_pend_qty" id="tot_pend_qty" type="hidden" value="<?=$tot_pendqty?>"/>
                   <input name="pid" id="pid" type="hidden" value="<?=$_REQUEST['pid']?>"/>
               	   <input name="hid" id="hid" type="hidden" value="<?=$_REQUEST['hid']?>"/>
                   <input title="Back" type="button" class="btn<?=$btncolor?>" value="Back" onClick="window.location.href='dispatch_pna_po.php?<?=$pagenav?>'">                 </td>
                </tr>
            </tbody>
          </table>
