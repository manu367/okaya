...
}else if($state != "ALL" && $location_code == ""){

	$locationid = " location_code in (select location_code from location_master where stateid ='".$state ."')";

}
else if($state == "ALl" && $location_code == ""){

	$locationid = " location_code in (select location_code from location_master where stateid ='".$arrstate."')";

}

else{

	$locationid=" location_code in (select location_code from location_master where stateid in(".$arrstate."))";
...
if($brandid !=""){
	...
	$brand_id=" brand_id in ($access_brand)";
}
/////get state///////////////
...
$i=2;
$count=1;
$sql_loc=mysqli_query($link1,"Select * from repair_detail where ($datetype >= '".$fromdate."'  and $datetype <='".$todate."') and ".$locationid." and ".$model_id." ");

while($row_loc = mysqli_fetch_array($sql_loc)){
$jobsheet = mysqli_fetch_array(mysqli_query($link1,"select call_for,call_type , warranty_status, customer_type,customer_name,contact_no,open_date,dop,product_id,brand_id,model_id,partcode from jobsheet_data where job_no ='".$row_loc['job_no']."'"));
...
$sql_sfr = mysqli_num_rows(mysqli_query($link1,"select *  from sfr_transaction where job_no = '".$row_loc['job_no']."' "));
if($sql_sfr == 0){
$sfr_flag="NORMAL JOBS";}else{ $sfr_flag="SFR JOBS";}
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('W'.$i, $jobsheet['partcode'])
			->setCellValue('X'.$i, getAnyDetails($jobsheet['partcode'],"part_desc","partcode","partcode_master",$link1))
...
header('Content-Disposition: attachment;filename="partconsume.xlsx"');
