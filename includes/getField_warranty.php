...
case chkModelSno:
   $level_query="SELECT model,model_id, wp,dwp,product_id,brand_id FROM model_master where modelcode='".$_REQUEST['value']."'  and status='1'";
$check2=mysqli_query($link1,$level_query);
    if(mysqli_num_rows($check2) > 0){
$br = mysqli_fetch_array($check2);
//  check  sale data
...
$sale_date="";
if($_REQUEST['value2']!=''){
 $level_2_query="SELECT activation_date FROM  imei_data_import where imei1='".$_REQUEST['value2']."'";
$serial_check=mysqli_query($link1,$level_2_query);
$serial_row = mysqli_fetch_array($serial_check);
$sale_date=$serial_row['activation_date'];
}

$product_name=getAnyDetails($br['product_id'],"product_name","product_id","product_master",$link1);
$barnd_name=getAnyDetails($br['brand_id'],"brand","brand_id","brand_master",$link1);
echo $br['model_id']."^".$br['model']."^".$br['wp']."^".$br['dwp']."^".$br['product_id']."^".$br['brand_id']."^".$product_name."^".$barnd_name."^".$_REQUEST['target']."^".$sale_date;
    }
else {
	
 //echo ""."^".""."^".""."^".$_REQUEST['target'];
 //echo "";
$level_3_query="SELECT * FROM  imei_data_import where imei1='".$_REQUEST['value2']."'";
$serial_check1=mysqli_query($link1,$level_3_query);
$serial_row1 = mysqli_fetch_array($serial_check1);
$level_query1="SELECT model,model_id, wp,dwp,product_id,brand_id FROM model_master where model_id='".$serial_row1['model_id']."'  and status='1'";
