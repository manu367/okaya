...
if($_REQUEST['job_status_sub']!=''){
	$sub_status=" and sub_status='".$_REQUEST['job_status_sub']."' ";
	}else{
		$sub_status= "";
		}
$columns = array(
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	if(($row["status"]==1 || $row["status"]==2 || $row["status"]==3 || $row["status"]==7 || $row["status"]==5 || $row["sub_status"]=="52" || $row["status"]=="54"|| $row["status"]=="56" || $row["status"]=="58" || $row["sub_status"]=="51" || $row["status"]=="18" || $row["status"]=="82")  && $row["eng_id"]!=""  ){
