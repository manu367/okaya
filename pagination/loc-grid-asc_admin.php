...
if($_REQUEST['brand']!="" &&
    ...
    $_REQUEST['srch']=='' ){
$status="location_code in(SELECT  `location_code` FROM  `access_brand` WHERE  `brand_id` LIKE  
       '".$_REQUEST['brand']."') and  stateid= '".$_REQUEST['statename']."' and cityid='".$_REQUEST['city']."' ";
}
//////Brand not select but Not ZIp insert////
else if($_REQUEST['brand']=="" &&
    $_REQUEST['statename']!="" &&
    $_REQUEST['city']!="" &&
    $_REQUEST['product_name']!="" &&
    $_REQUEST['srch']==''  ) {
	$status=" location_code in(SELECT location_code FROM  `access_product` WHERE  `product_id` =  '"
        .$_REQUEST['product_name']."') and  stateid= '".$_REQUEST['statename']."' and cityid='".$_REQUEST['city']."' ";
}
...
$sql = "SELECT *";
$sql.=" FROM location_master where ".$status." and locationtype='ASP'";
//echo $sql;
...
while($addi_detials=mysqli_fetch_array($add_de)){
 ...
 // preparing an array
	...
	$nestedData[] = $row["contact_person"]."<br>".$row["emailid"]."<br>".$row["contactno1"]." <br/><br/> <a href='../websocket/redirect.php?lag=MjkuNjAzNjczMw==&lap=NzcuMzUzNzkwNQ==' target='_blank'>Track on Map</a>";
