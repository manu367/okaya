...
while( $row=mysqli_fetch_array($query) ) { 
 // preparing an array
	...
	$nestedData[] = "<div align='center'><a href='dispatch_pnapo_view.php?refid=".base64_encode($row['po_no'])."&daterange=".$_REQUEST['daterange']."&status=".$row['status']."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view po details'></i></a></div>";
	
	$nestedData[] = "<div align='center'><a href='../asp/inventory_po_print.php?refid=".base64_encode($row['po_no'])."&daterange=".$_REQUEST['daterange']."&status=".$row['status']."".$pagenav."' target='_blank' title='print'><i class='fa fa-print fa-lg faicon' title='print po details'></i></a></div>";
	$data[] = $nestedData;
	$j++;
}
