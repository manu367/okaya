...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$statearea = 	$cust_st[0];
	$stcode=$cust_st[1];
	$aspcode=$_SESSION['asc_code'];
	//$othid=$_SESSION['othid'];
	
	// upadte  job counter ASP wise-
	//$res_jobcount = mysqli_query($link1,"SELECT job_counter  from date_counter  where  job_date ='".$today."'");
	$res_jobcount = mysqli_query($link1,"SELECT job_count  from job_counter  where  location_code ='".$aspcode."'");
	...
	//$jobno = $stcode."".$job_dt."".str_pad($nextjobno,4,0,STR_PAD_LEFT);
		...
		$jobno="J".$job_dt."".$aspcode."".str_pad($nextjobno,4,0,STR_PAD_LEFT);
	//// first update the job count
	$res_upd = mysqli_query($link1,"UPDATE job_counter set job_count='".$nextjobno."' where location_code ='".$aspcode."'");
	}else {
		$nextjobno=1;
		$job_dt=date("ymd");
		//$jobno=$stcode."".$job_dt."".str_pad($nextjobno,4,0,STR_PAD_LEFT);
		...
		$jobno="J".$job_dt."".$aspcode."".str_pad($nextjobno,4,0,STR_PAD_LEFT);
		$res_upd = mysqli_query($link1,"insert into job_counter set job_count='".$nextjobno."',location_code ='".$aspcode."'");
	}
...
$st_wart=$warranty_status;
 ...
 $unit_part=mysqli_query($link1,"select partcode from partcode_master where   model_id Like '%".$modelsplit[0]."%' and status='1' and  	part_category='UNIT'" )or die(mysqli_error($link1)); 
		...
		/*	if($row_part['partcode']==""){
			 $flag = false;
			 $error_msg = "Partcode Not found in partcode master please check : " .$modelsplit[0]. ".";
			}
*/
...
var wday= parseInt(modelsplit[2]);
  ...
  function getSerailInfo(serial){
	  ...
	  $.ajax({
		...
		data:{serialinfo:serial},

		success:function(data){
	
		document.getElementById("out_flag").value=data;
...
// check black listed imei


var imei_serailvalue=document.getElementById("imei_serial1").value;
var out_flag="";
if(imei_serailvalue ==''){
alert("IMEI/Serial No. is black, Please enter first, then select  Purchase date.");
}else{

getSerailInfo(imei_serailvalue);
out_flag=document.getElementById("out_flag").value;
	...
	//alert(diffDays+""+post_wsd);
		if(diffDays <= tat_warrty && out_flag!='Y'){
...
//// date difference
	...
	// getting brand mapped product -ravi
	
		function getmapproduct_brand(){
	  ...
	  $.ajax({
	    type:'post',
		url:'../includes/getAzaxFields.php',
		data:{brandproductmap:brand1,rep_location:curent_loc},
		success:function(data){
              	...
              	<div class="form-group">
                    ...
                    <div class="col-md-6"><label class="col-md-6 custom_label">Brand <span class="red_small">*</span></label>
                      ...
                      <div class="col-md-6" id="selectedbrand">
                       ...
                       <select name="brand" id="brand" class="form-control  selectpicker required" data-live-search="true" onChange="getmapproduct_brand(this.value);" required> <!--getmapbrand(this.value);--->
                      ...
                      <div class="col-md-6" id="selectedproduct">

                         <select name="product_name" id="product_name" class="form-control selectpicker" data-live-search = "true" required ><!--getmapproduct(this.value);-->
              	...
              	<div class="form-group">
                    ...
                    <div class="col-md-6"><label class="col-md-6 custom_label">Model <span class="red_small">*</span></label>
                      ...
                      <div class="col-md-6" id="modeldiv">
                        ...
                        <select name="modelid" id="modelid" class="form-control selectpicker required" data-live-search="true" required>
...
echo $model_det2['7'];
                    ...
                    <div class="col-md-6"><label class="col-md-6 custom_label"><?php echo SERIALNO ?> <span class="red_small">*</span> </label>
                      ...
                      <div class="col-md-6" >
<?php
if($product_det['serial_no']!=""){$imei_ser=$product_det['serial_no'];}else{$imei_ser=$_REQUEST['imei_serial'];}
?>
	<input name="imei_serial1" id="imei_serial1" type="text" value="<?=$imei_ser?>" class="required form-control  " onKeyUp="getdate4();"   <?php if($product_det['serial_no']!=''){?> readonly <?php }else{}?>  />
    <input name="out_flag" id="out_flag" type="hidden" value=""  />
                      </div>
  ...
  <div style="display:inline-block;float:left;"><input type="text" class="form-control " name="install_date"  id="install_date" style="width:150px;" value="<?=$product_det['installation_date'];?>"   readonly  ></div><div style="display:inline-block;float:left;"><i class="fa fa-calendar fa-lg"></i></div>
                ...
                <!-- <div class="form-group">
				   ...
				   <div class="form-group">
                    ...
                    <div class="col-md-6"><label class="col-md-6 custom_label">Invoice No <span class="red_small">*</span> </label>

                      <div class="col-md-6">

                      <input name="invoice_no" id="invoice_no" type="text" value="" class="form-control required" required/>
                      ...
                      <div class="col-md-6" id="accdiv">
                       ...
                       <select name="acc_present[]" id="example-multiple-selected2" multiple="multiple" class="form-control">
						<?php  // $acc_part = mysqli_fetch_assoc(mysqli_query($link1,"select partcode,part_name from partcode_master where model_id='".$model_code."' and part_category='ACCESSORY' and status='1'"));
						  ...
						  ?>
<option value="<?php //$br_acc['part_name']?>"><?php // echo $br_acc['part_name']?></option>
				  ...
				  <div class="form-group">
					  ...
					  <select name="rep_location" id="rep_location" class="form-control required" required>
                       ...
                       $lctype_query="select location_code,locationname  from location_master where  location_code = '".$_SESSION['asc_code']."' order by locationname";
                        ...
                        <?php } ?>
						 <!--<option value="RVS"<?php if($_REQUEST['rep_location']=="RVS"){ echo "selected";}?>><?php echo "CS Pvt Ltd"; ?></option>--->
                      </select>
                      </div>
                    </div>
                  ...
                  <div class="form-group" id="vocdisplay" >
                    ...
                    <div class="col-md-6"><label class="col-md-6 custom_label">VOC <span class="red_small">*</span></label>
                      ...
                      <div class="col-md-6" id="vocdiv">
						<?php
							 $vocpro="SELECT * FROM voc_master where product_id='".$product_det['product_id']."' and status='1' group by voc_desc order by voc_desc ";
								?>
	  ...
	  <!------------- Start Image Uploder --------------->
		  <div class="form-group">
			<label class="col-md-3 custom_label">Any Image/Attachment <span class="red_small">*</span></label>
			  <div class="col-md-5">
				  <input type="file"  name="handset_img" id="handset_img" onChange="return validateImage('handset_img','0');" class="form-control "  accept=".png,.jpg,.jpeg,.gif" required /><br>
