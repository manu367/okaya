...
@extract($_POST);
	if($_POST['savermk']=='Save'){
		...
		/////////////////////  entry in call history table ///////////////////////////
	 	$query="update jobsheet_data set hc_feedback='".$proiority."', hc_remark='".$remark."',hc_reason='".$reason."' where job_no='".$docid."'";
	...
	////// insert in activity table////
		$flag = dailyActivity($_SESSION['userid'],$docid,"Happy Calling","UPDATE",$_SERVER['REMOTE_ADDR'],$link1,$flag);
	///// check query are successfully executed
	if ($flag) {
		$cflag="success";

		$cmsg="Success";
		$msg = "Happy calling remark is successfully updated against job no.".$docid;
	    mysqli_commit($link1);
	} else {
   ...
   ///// move to parent page
header("location:happy_calling_list.php?msg=".$msg."&chkflag=".$cflag."&chkmsg=".$cmsg."".$pagenav);
  exit;
}
}
      ...
      <div class="panel-body">
           <table class="table table-bordered" width="100%">
            ...
            <tr>
                ...
                <td width="30%"><select name="proiority" id="proiority" class="form-control required"  required style="width:200px;">
                          <option value=''>--Select Feedback--</option>
						   <option value='0'>Customer Not available</option>
			   ...
			   <tr>
              <td><label class="control-label">Reasons for choosing the rating</label></td>
              <td>
			  
			   <select name="reason" id="reason" class="form-control required" style="width:200px;"  >
                   <option value="">Please Select</option>
                   <?php
                $fed= mysqli_query($link1,"select * from feedback_master where 1"); 
                while( $feedback = mysqli_fetch_assoc($fed)){
				
				?>
                   <option value="<?=$feedback['sno']?>" <?php if($_REQUEST['reason'] == $feedback['sno']) { echo 'selected'; }?>>
                  <?=$feedback['feedback']?>
                  </option>
                   <?php } ?>
                 </select></td>
            ...
            <tr>
              <td><label class="control-label">Remark</label></td>
              <td><textarea name="remark" id="remark" required class="form-control" onkeypress = " return ( (event.keyCode ? event.keyCode : event.which ? event.which : event.charCode)!= 13);"  onContextMenu="return false" style="resize:none" style="width:200px;"></textarea></td>
            </tr>   
		   ...
		   <div class="form-group">
                    <div class="col-md-12" align="center">
					     <input title="Back" type="button" class="btn<?=$btncolor?>" value="Back" onClick="window.location.href='happy_calling_list.php?<?=$pagenav?>&mobileno=<?=$job_row['contact_no']?>&customer_id=<?=$job_row['customer_id']?>&email_id=<?=$job_row['email']?>&imei_serial=<?=$job_row['imei']?>'">
