...
############################################ Symptom Code #######################################################
$rs_symp=mysqli_query($link1,"select * from symptom_master where status='1'  and  prod_cat like '%".$job_row['product_cat']."%'  order by symp_desc")or die("Error-> in symptom code".mysqli_error($link1));
...
$rs_sec=mysqli_query($link1,"select * from section_master where status='1'  and  prod_cat like '%".$job_row['product_cat']."%' order by  	section_desc ")or die("Error-> in section code".mysqli_error($link1));
...
//////////////////////////////////////////////////////
$rs_def=mysqli_query($link1,"select * from defect_master where status='1'  and  prod_cat like '%".$job_row['product_cat']."%' order by  	defect_desc  ")or die("Error-> in defect code".mysqli_error($link1));
...
function change_stat(val){
	...
	if(val=='5'){
		...
		document.getElementById("repl_reason").style.display="none";
		document.getElementById("reason_repl").value="";
	...
	/*else if(val=='8'){
		...
		if("<?=$model_flags[2]?>" == "Y"){
			...
			var modelreplcedays = "<?=$model_flags[3]?>";
				...
				document.getElementById("errmsg").innerHTML = "";
				document.getElementById("savejob").style.display="";
				document.getElementById("repl_reason").style.display="";
...
function getWarranty(){
	var sel_pop = "<?=$job_row['dop']?>";
	var sel_elstatus = $('#els_status').val();
	var post_wsd = "<?=$model_flags[5]?>";
	var post_w = "<?=$job_row['warranty_status']?>";
	...
	if("<?=$model_flags[4]?>" == "Y"){
		...
		if(diffDays <= post_wsd){
			...
			}else if(sel_elstatus != "OK"){
				document.getElementById("warranty_status").value = "VOID";
			}else{
				document.getElementById("warranty_status").value = post_w;
			}
		}else{
			document.getElementById("warranty_status").value = "OUT";
		}
...
function addNewRow(findex){
    ...
    //// check if previous row is filled with these three values (Problem Observed,Solution Given, part)/////
    //if(document.getElementById(faultcode).value!="" && document.getElementById(repaircode).value!="" && document.getElementById(partcode).value!=""){
	   ...
	   newdiv.innerHTML = '<i class="fa fa-close fa-lg" onclick="return getRowDisable('+num+')" title="Remove this row"></i><table width="auto" class="table table-bordered"><tbody><tr class="<?=$tableheadcolor?>"><td width="20%"><strong>Condition</strong></td><td width="20%" ><strong>Symptom</strong></td><td width="20%" ><strong>Section</strong></td><td width="20%"><strong>Problem Observed</strong></td></tr><tr><td width="25%"><select name="con_code['+num+']" id="con_code['+num+']" style="width:230px;" class="form-control"><option value="" selected="selected">Select Condition</option><?php  $zk=0; while($zk<count($cond_arr)){?><option value="<?=$cond_arr[$zk][0]?>"><?=$cond_arr[$zk][1]?> (<?=$cond_arr[$zk][0]?>)</option><?php $zk++;}?></select></td><td width="25%"><select name="sym_code['+num+']" id="sym_code['+num+']" style="width:230px;" class="form-control"><option value="" selected="selected">Select Symptom </option><?php  $z=0; while($z<count($symp_arr)){?><option value="<?=$symp_arr[$z][0]?>"><?=$symp_arr[$z][1]?> (<?=$symp_arr[$z][0]?>)</option><?php $z++;}?></select></td><td width="25%"><select name="sec_code['+num+']" id="sec_code['+num+']" style="width:230px;" class="form-control"><option value="" selected="selected">Select Section </option><?php  $jk=0; while($jk<count($sec_arr)){?><option value="<?=$sec_arr[$jk][0]?>"><?=$sec_arr[$jk][1]?> (<?=$sec_arr[$jk][0]?>)</option><?php $jk++;}?></select></td><td width="25%"><select name="fault_code['+num+']" id="fault_code['+num+']" style="width:230px;" class="form-control" onChange="getRepairDropDown(this.value,'+num+')"><option value="" selected="selected">Select Defect</option><?php  $f=0; while($f<count($def_arr)){?><option value="<?=$def_arr[$f][0]?>"><?=$def_arr[$f][1]?> (<?=$def_arr[$f][0]?>)</option><?php $f++;}?></select></td></tr><tr class="<?=$tableheadcolor?>"><td width="20%"><strong>Solution Given</strong></td><td width="20%"><strong>Part</strong></td><td width="10%"><strong>Price</strong></td><td width="10%"><strong>Available QTY</strong></td></tr><tr><td width="25%"><span id="repDiv'+num+'"><select name="repair_code['+num+']" id="repair_code['+num+']" style="width:230px;" class="form-control" disabled="disabled" onChange="return getPartDropDown(this.value,'+num+');"><option value="" selected="selected"> Select Solution Given</option></select></span><input name="repair_level['+num+']" id="repair_level['+num+']" class="form-control" type="hidden" readonly/></td><td width="30%"><span id="partDiv'+num+'"><select name="part['+num+']" id="part['+num+']" disabled="disabled" style="width:300px" class="form-control" onChange="getstockable(this.value,'+num+');"><option value="" selected="selected"> Select Part</option></select></span> <div id="dispswapimeifield['+num+']" style="display:none;"><input name="swap_imei1['+num+']" id="swap_imei1['+num+']" type="text"  class="form-control alphanumeric"  maxlength="15" style="width:200px;" placeholder="Enter IMEI1/Serial No.1"/><input name="swap_imei2['+num+']" id="swap_imei2['+num+']"  type="text"  style="width:200px;" class="form-control alphanumeric" placeholder="Enter IMEI2/Serial No.2"/></div></td><td width="10%"><input name="part_price['+num+']" id="part_price['+num+']" class="form-control" type="text" style="width:100px;text-align:right" readonly/></td><td width="10%"><input name="avlqty['+num+']" id="avlqty['+num+']" class="form-control" type="text" style="width:100px;text-align:right" readonly/></td></tr></tbody></table>';
...
function addNewRowPna(findex){
   var ni = document.getElementById('myDivPna');
    ...
    //// check if previous row is filled with these three values (Problem Observed,Solution Given, part)/////
       ...
       var newdiv = document.createElement('div');
       newdiv.setAttribute("id",divIdName);
	   newdiv.innerHTML = "<i class='fa fa-close fa-lg' onclick='return getRowDisablePna("+num+")' title='Remove this row'></i><table width='100%' class='table table-bordered'><tbody><tr><td><select name='pending_part["+num+"]' class='form-control' id='pending_part["+num+"]' style='width:250px;'  onChange='duplicate_entry("+num+",this.value)';><option value='' selected='selected'> Select Pending Part</option><?php $rs_pna = mysqli_query($link1,"select partcode, part_name, part_category from partcode_master  where  model_id like '%".$job_row['model_id']."%' and status='1'      and partcode not in  (  select partcode from user_inventory  where location_code='".$_SESSION['asc_code']."'  and  	locationuser_code='".$job_row['eng_id']."' and okqty > 0)  order by part_name")or die("error in pna dropdown".mysqli_error($link1));while($row_pna = mysqli_fetch_array($rs_pna)){?><option value='<?=$row_pna[0]?>'><?=$row_pna[1]."-".$row_pna[2]?> (<?=$row_pna[0]?> ) </option><?php }?></select><span id='errmsg["+num+"]' name='errmsg["+num+"]' class='red_small'></span></td></tr></tbody></table>";
       ni.appendChild(newdiv);
	    if(num==4){ 
...
function addNewRowdemo(findex){
   var ni = document.getElementById('myDivDemo_install');
    ...
    //// check if previous row is filled with these three values (Problem Observed,Solution Given, part)/////
       ...
       var newdiv = document.createElement('div');
       newdiv.setAttribute("id",divIdName);
	   newdiv.innerHTML = "<i class='fa fa-close fa-lg' onclick='return getRowDisabledemo("+num+")' title='Remove this row'></i><table width='100%' class='table table-bordered'><tbody><tr><td><select name='req_part["+num+"]' class='form-control' id='req_part["+num+"]' style='width:250px;'  onChange='duplicate_entry("+num+",this.value)';><option value='' selected='selected'> Select  Part</option><?php $rs_as = mysqli_query($link1,"select partcode, part_name, part_category from partcode_master  where  model_id like '%".$job_row['model_id']."%' and status='1'      and partcode  in  ( select partcode from user_inventory  where location_code='".$_SESSION['asc_code']."' and locationuser_code='".$job_row['eng_id']."' and  okqty > 0  ) and part_category='ACCESSORY' order by part_name")or die("error in pna dropdown".mysqli_error($link1));while($row_as = mysqli_fetch_array($rs_as)){?><option value='<?=$row_as[0]?>'><?=$row_as[1]."-".$row_as[2]?> (<?=$row_as[0]?> ) </option><?php }?></select><span id='errmsg["+num+"]' name='errmsg["+num+"]' class='red_small'></span></td></tr></tbody></table>";
       ni.appendChild(newdiv);
	    if(num==4){ 
...
/////////// function to get Solution Given on the basis of symptom code
  function getRepairDropDown(val,indx){
	  $.ajax({
	    type:'post',
		url:'../includes/getAzaxFields.php',
		data:{symptomcode:val, replevel:'<?=$replvl?>', indxno:indx,product_cat:'<?=$job_row['product_cat']?>'},
		success:function(data){
...
/////////// function to get part on the basis of Solution Given
   ...
   var part_det="";
    ...
    var totrows=document.getElementById('theValue').value;
	  ...
	  $.ajax({
	    type:'post',
		url:'../includes/getAzaxFields.php',
		data:{repaircode:val, modelcode:'<?=$job_row['model_id']?>',engid:'<?=$job_row['eng_id']?>', locationcode:'<?=$_SESSION['asc_code']?>', indxno:indx, part_inf:part_det},
		success:function(data){
		var splitPart=data.split("~");
	  	...
	  	if(splitPart[0]!="" && splitPart[0]!=0 && splitPart[2]=="Y"){
			...
			if(val=='R0086'){
		 	...
		 	///// assign class for part mandatory if Solution Given selected /////////
  ...
  function replace_type(val){
	  $.ajax({
	    type:'post',
		url:'../includes/getAzaxFields.php',
		data:{replacemodel:val, locationcode:'<?=$_SESSION['asc_code']?>'},
		success:function(data){
		...
		success:function(data){
			...
			if(data==0 || data=='' || data < 0){
				...
				if(sim_type == "C" || sim_type == "G"){
					...
					//document.getElementById("new_imei1").onblur = function() {get_sec_imei(this.value)};
 ...
 function validateImage(nam,ind) {
  ...
  <div class="container-fluid">
        ...
        <div class="form-group"  id="page-wrap" style="margin-left:10px;" >
          ...
          <div class="tab-content">
            ...
            <div class="form-group">
                ...
                <div class="col-md-6"><label class="col-md-6">Accessory Required</label>
                  <div class="col-md-6">
                  <?php 	$acc= explode(",",$job_row['acc_rec']); 
                ...
                <div class="col-md-6"><label class="col-md-5">Repair Status <span class="red_small">*</span></label>
                  <div class="col-md-6">
                    <select name="jobstatus" id="jobstatus" onChange="return change_stat(this.value);" class="required form-control" style="width:250px;" required>
                      ...
                      <?php if($job_row['call_for']=='Installation' || $job_row['call_for']=='Demo' ){
					   $res_jobstatus = mysqli_query($link1,"select status_id, display_status from jobstatus_master where system_status in ('Demo Done','Installation Done','WIP','Request For Approval') order by display_status");
						}else{
						if($job_row['status']=='58'){
						   $res_jobstatus = mysqli_query($link1,"select status_id, display_status from jobstatus_master where status_id=main_status_id and system_status in ('Repair Done') order by display_status");
						   }else{
						      $res_jobstatus = mysqli_query($link1,"select status_id, display_status from jobstatus_master where status_id=main_status_id and system_status in ('EP','PNA','Repair Done','RWR','SFR','WIP','Request For Approval','Repair at Field') order by display_status");
						   }
						}	
                      ...
                      <?php
							...
							if($job_row['app_reason']=='Call cancellation' &&  $job_row['doa_approval']=='Y')
							{?>
					    <option value="12">Cancel</option>
							
							
						<?php 	}
							if($job_row['app_reason']=='Replacement' &&  $job_row['doa_approval']=='Y')
							{?>
           ...
           <?php if($job_row['area_type']=='Upcountry'){?>     <div class="col-md-6"><label class="col-md-5">Travel By Engineer<span class="red_small">(IN KM)</span></label>
			...
			<div class="form-group" id="close_reason" style="display:none">
                ...
                <div class="col-md-6"><label class="col-md-5"></label>
			  	...
			  	<div class="form-group" >
                <div class="col-md-6"><label class="col-md-5"> Jobsheet </label>
                  <div class="col-md-6">
				    <input type="file"  name="handset_img" id="handset_img" onChange="return validateImage('handset_img','0');" class="form-control "  accept=".png,.jpg,.jpeg,.gif" />
                ...
                <div class="col-md-6"><label class="col-md-6"></label>
                  <div class="col-md-6">
			  			  	...
			  			  	<div class="form-group" >
                <div class="col-md-6"><label class="col-md-5"> Warranty Card/Invoice Copy </label>
                  <div class="col-md-6">
                ...
                <div class="col-md-6"><label class="col-md-6"> Product serial No. </label>
                  <div class="col-md-6">
                  <input type="file"  name="handset_img3" id="handset_img3" onChange="return validateImage('handset_img3','0');" class="form-control "  accept=".png,.jpg,.jpeg,.gif" />
			  			  			  	...
			  			  			  	<div class="form-group" >
                <div class="col-md-6"><label class="col-md-5"> Product Photo 1 </label>
                  <div class="col-md-6">
                ...
                <div class="col-md-6"><label class="col-md-6"> Product Photo 2 </label>
                  <div class="col-md-6">
              	...
              	<div class="form-group" id="repl_reason" style="display:none">
                <div class="col-md-6"><label class="col-md-5">Replacement Reason</label>
                  <div class="col-md-6">
				   <select name="reason_repl" id="reason_repl"  class="form-control" style="width:250px;" >
				   <option value="" >--Select Replacement--</option>
                 <option value="Dead on arrival" >Dead on arrival</option>
				   <option value="Repeat failure" >Repeat failure</option>
				      <option value="Spare part not available" >Spare part not available</option>
					    <option value="Delay in service" >Delay in service</option>
						 <option value="Customer not ready for repair">Customer not ready for repair</option>
                  <option value="Product quality issue">Product quality issue</option> 
                  <option value="Product upgrade">Product upgrade</option>                       
                        </select>
                    ...
                    <table width="100%" class="table table-bordered">
                      <thead>
                        <tr class="<?=$tableheadcolor?>">
						  <td width="20%"><strong>Condition</strong></td>
						    <td width="20%" ><strong>Symptom</strong></td>
							 <td width="20%" ><strong>Section</strong></td>
							
                     <td width="20%"><strong>Problem Observed</strong></td>
                      ...
                      <tbody>
                        <tr>
						 <td>
                              <select name="con_code[0]" id="con_code[0]" style="width:230px;" class=" form-control">
                                  <option value="" selected="selected">Select Condition</option>
                                  <?php  $zk=0; while($zk<count($cond_arr)){?>
                                    <option value="<?=$cond_arr[$zk][0]?>"><?=$cond_arr[$zk][1]?> (<?=$cond_arr[$zk][0]?>)</option>
                                  <?php $zk++;}?>
                              </select>
                          </td>
			      <td >
                              <select name="sym_code[0]" id="sym_code[0]" style="width:230px;" class=" form-control">
                                  <option value="" selected="selected">Select Symptom Code</option>
						     ...
						     <td>
                              <select name="sec_code[0]" id="sec_code[0]" style="width:230px;" class=" form-control">
                                  <option value="" selected="selected">Select Section</option>
                                  <?php  $jk=0; while($jk<count($sec_arr)){?>
                                    <option value="<?=$sec_arr[$jk][0]?>"><?=$sec_arr[$jk][1]?> (<?=$sec_arr[$jk][0]?>)</option>
                                  <?php $jk++;}?>
                              </select>
                          </td>
						...
						<tr class="<?=$tableheadcolor?>">     
                          <td width="20%"><strong>Solution Given</strong></td>
                          ...
                          <td>
                            ...
                            <select name="repair_code[0]" id="repair_code[0]" style="width:230px;" class="required form-control" disabled="disabled" onChange="return getPartDropDown(this.value,'0');">
                            	<option value="" selected="selected"> Select Solution Given</option>
                          	</select>
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td width="30%"><select name="send_for" id="send_for" class="required form-control" style="width:250px;" required>
                              ...
                              <?php
    							$rs_sendfor = mysqli_query($link1,"select location_code, locationname, cityid, stateid from location_master  where location_code in (select repair_location from map_repair_location where location_code='".$_SESSION['asc_code']."' and status='Y') and statusid='1'")or die("error2".mysqli_error($link1));
    ...
    ?>
                        ...
                        <tr>
                          <td width="20%"><strong>Problem Observed <span class="red_small">*</span></strong></td>
                          <td width="30%"><select name="fault_code_sfr" id="fault_code_sfr" style="width:250px;"  class="required form-control" >
                              <option value="" selected="selected">Select Problem Observed</option>
                              <?php  $z=0; while($z<count($symp_arr)){?>
                              <option value="<?=$symp_arr[$z][0]?>">
                              <?=$symp_arr[$z][1]?> (<?=$symp_arr[$z][0]?>)</option>
                              <?php $z++;}?>
                            </select></td>
                          <td width="20%"><strong>Solution Given <span class="red_small">*</span></strong></td>
                  		...
                  		<tr>
                    		<td>
                    			<select name="req_part[0]" class=" form-control"  id="req_part[0]" style="width:250px;">
									...
									$rs_as = mysqli_query($link1,"select partcode, part_name, part_category from partcode_master  where  model_id like '%".$job_row['model_id']."%'  and status='1'  and part_category='ACCESSORY'     and partcode  in  ( select partcode from user_inventory  where location_code='".$_SESSION['asc_code']."' and locationuser_code='".$job_row['eng_id']."' and okqty > 0)   order by part_name")or die("error in pna dropdown".mysqli_error($link1));
									
									while($row_as = mysqli_fetch_array($rs_as)){
									?>
                        			<option value="<?=$row_as[0]?>"><?=$row_as[1]."-".$row_as[2]?> (<?=$row_as[0]?> ) </option>
                        			<?php
                        			}	
                        			?>
      							</select>
                 ...
                 <tbody>  
                  ...
                  <div id="replacement" style="display:none"><br/>
                      ...
                      <tbody>  
                		<tr>
                  			<td>
                            	<select name="fault_code_replace" id="fault_code_replace" style="width:200px;"  class=" form-control">
                      				<option value="" selected="selected">Select Problem Observed</option>
                      				<?php  $z=0; while($z<count($symp_arr)){?>
                      				<option value="<?=$symp_arr[$z][0]?>"><?=$symp_arr[$z][1]?> (<?=$symp_arr[$z][0]?>)</option>
                      				<?php $z++;}?>
                  			...
                  			<td>
                    			<select name="repair_code_replace" id="repair_code_replace" style="width:200px;" class=" form-control">
                                 	<option value="" selected="selected"> Select Solution Given</option>
                                   	<?php 
                  			...
                  			<td>
								<?php
                                $sql_model = "SELECT model_id, model FROM model_master where replacement='Y' and status='1'  and product_id='".$job_row['product_id']."' order by model";
                                $res_model = mysqli_query($link1,$sql_model);
                                ?>
                              	<select  name="replace_model" class=" form-control" id="replace_model" style="width:150px;" >
                                	<option value="" >Please Select</option>
                                <?php
                            ...
                            <td>
                  			...
                  			<td id="dispimeifield" >
                            	<input name="new_imei1" id="new_imei1" type="text" class=" form-control alphanumeric"  maxlength="15" style="width:200px;" placeholder="<?php echo SERIALNO ?>"/>
