...
$error_msg="";   
   ...
   //////////////////////////////// Insert call  history//////////////////////////////////////
    $flag2 = callHistory($_POST['job_no'],$_SESSION['userid'],"1","Re-Assign(location and eng)","Re-Assign(location and eng)",$_SESSION['userid'],$ws,"","","",$ip,$link1,$flag);

	  
	  /////////////////////////////////////Job Sheet data////////////////////////////////////////////////////////
	 //echo "update jobsheet_data set current_location='".$_REQUEST['rep_location']."' and eng_id='".$_REQUEST['rep_eng']."' ,doa_rej_rmk='',ack_remark='' where job_no='".$_POST['job_no']."'";exit;
   $up_job=mysqli_query($link1,"update jobsheet_data set current_location='".$_REQUEST['rep_location']."',eng_id='".$_REQUEST['rep_eng']."' ,doa_rej_rmk='',ack_remark='' where job_no='".$_POST['job_no']."'");
 ...
 header("location:reject_jobs_list_admin.php?msg=".$msg."&chkflag=".$cflag."&chkmsg=".$cmsg."".$pagenav);
            ...
            <tr>
              ...
              <td width="51%"  id="loc_pincode">
               ...
               ?>
				 ...
				 <tr>
              <td width="49%"><label class="control-label">Reassigen Job(ENG) </label></td>
              <td width="51%"  id="loc_pincode1"> <!--<select   id="rep_location" name="rep_location" class="form-control required" required >
          
                        <option value="">--Please Select--</option>
                      
						
                      </select>-->
				  <select  name='rep_eng' id='rep_eng' class='form-control required select2' required>
               <?php
			          
                      //echo "<select  name='rep_location' id='rep_location' class='form-control required select2'>";

		//and location_code='".$_SESSION['asc_code']."'
		$pin_loc1="select * from locationuser_master where location_code='".$_REQUEST['rep_location']."' and statusid='1'  order by locusername  ";
		$loc_pin1=mysqli_query($link1,$pin_loc1);
		echo "<option value=''>--Please Select--</option>";
		while($loc_cpin1 = mysqli_fetch_array($loc_pin1)){
			...
			echo "<option value='".$loc_cpin1['userloginid']."'>";
			echo $loc_cpin1['userloginid']." | ".$loc_cpin1['locusername']."</option>";
